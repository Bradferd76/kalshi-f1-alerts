...
def fetch_broad_market_sample():
    """
    Fetch a broad sample of open Kalshi markets.

    This provides an additional discovery source for F1 markets.
    """
    try:
        response = requests.get(
            MARKETS_URL,
        ...
        print(
            f"Error fetching broad market sample: {error}"
        )
        return []
...
def market_matches_type(market, market_type):
    """
    Determine whether a market matches Top 5, Top 10, podium,
    winner, or pole.
    """
    ...
    if normalized_type == "top 10":
        ...
        if (
            series_ticker == "KXF1TOP10"
            or ticker.startswith("KXF1TOP10-")
            or event_ticker.startswith("KXF1TOP10-")
        ):
            return True
    ...
    if normalized_type == "top 5":
        ...
        if (
            series_ticker == "KXF1TOP5"
            or ticker.startswith("KXF1TOP5-")
            or event_ticker.startswith("KXF1TOP5-")
        ):
            return True
...
def market_matches_driver(market, driver_code):
    """
    Match a driver using the final ticker segment
    or market text.
    """
    ...
    # Examples:
    #
    # KXF1TOP10-SPAGP26-ANT
    # KXF1TOP5-DUTGP26-ANT
    ...
    driver_name = driver_name_by_code.get(
        driver_code.lower()
    )
...
def find_matching_markets(
    ...
    """Find markets matching market type and driver."""
...
def get_price_cents(market, side):
    """Get YES or NO ask price in cents."""
...
def check_price_alerts():
    ...
    alert_state = load_json_file(
        PRICE_ALERT_STATE_FILE,
        {}
    )
    ...
    # --------------------------------------------------------
    # Fetch all relevant F1 series
    # --------------------------------------------------------

    all_markets = []

    known_series_to_check = {
        "KXF1RACE",
        "KXF1RACEPODIUM",
        "KXF1POLE",
        "KXF1TOP5",
    ...
    # Also include broad market discovery.
    ...
    all_markets = deduplicate_markets(
        all_markets
    )

    print(
        "Total unique markets available "
        f"for price checks: {len(all_markets)}"
    )
    ...
    for alert_number, alert in enumerate(
        ...
        # ----------------------------------------------------
        # Restrict search to the correct series when known.
        # ----------------------------------------------------

        series_ticker = KNOWN_SERIES.get(
            market_type.lower()
        )
        ...
        if not matching_markets:
            ...
            if possible_f1_markets:
                ...
                print(
                    "Potential F1 markets available "
                    "for inspection: "
        ...
        for market in matching_markets:
            ...
            # Alert when ask is at or below threshold.
            threshold_reached = (
                current_cents <= target_cents
            )
