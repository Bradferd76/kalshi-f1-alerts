...
```
This is used as an additional discovery source for F1 markets.
"""
try:
    response = requests.get(
        MARKETS_URL,
...
ticker because Kalshi may not expose series_ticker in every response.
"""
...
# --------------------------------------------------------
# Start with the known F1 series.
# --------------------------------------------------------
...
known_series_to_check = {
    ...
    # These are the correct Kalshi F1 Top 5 / Top 10 series.
    "KXF1TOP5",
    "KXF1TOP10",
}
...
# Add a broad sample as an additional discovery source.
...
print(
    f"Total unique markets available for price checks: "
    f"{len(all_markets)}"
)
...
for alert_number, alert in enumerate(alerts, start=1):
    ...
    # If the market type has a known dedicated series,
    # prioritize markets from that series.
    ...
    for market in matching_markets:
        ...
        # Alert when the ask is at or below
